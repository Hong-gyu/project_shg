...
public class CalenderTest {
	...
	public static void main(String[] args) {
		...
		int year = now.get(Calendar.YEAR); // now.get(1)과 같음
		int month = now.get(Calendar.MONTH)+1; // now.get(2)와 같음, +1의 이유는 month는 0에서 11의 숫자를 가지기 때문(+1을 해줘야 1~12월으로 표기할 수 있음)
		...
		int amPm = now.get(Calendar.AM_PM); // 오전 또는 오후 인지 확인
