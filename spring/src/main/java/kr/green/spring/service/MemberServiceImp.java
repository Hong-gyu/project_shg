...
public class MemberServiceImp implements MemberService {
	...
	public boolean signup(MemberVO user) {
		...
		user.setPw(encodePw);
		// 없으면 다오에게 회원정보를 주면서 회원가입하라고 시킨 후 true 리턴
	...
	@Override
	public MemberVO mypage(MemberVO id) {
	...
	public MemberVO updateMember(MemberVO user) {
		...
		MemberVO dbUser = memberDao.getMember(user.getId());
		// 일치하는 회원정보가 없으면 0을 반환
		if(dbUser == null) {
			return null;
		}
		// 기존 회원 정보 중 서별, 이메일을 수정할 회원 정보의 성별, 이메일로 변경
