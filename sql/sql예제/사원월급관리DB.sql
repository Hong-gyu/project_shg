/* 사원들의 월급을 관리하는 시스템을 위한 DB를 설계하시오
...
create table if not exists salary (
    ...
    sa_em_base_salary int not null default 0,
    sa_em_add_salary int not null default 0,
...
insert into salary
	values ('사원',200,10);
insert into salary
	values ('대리',220,15), ('과장',240,20), ('부장',280,25), ('전무',300,30), ('이사',350,35);
insert into employee
	values (1, '홍길동', '사원', 2, 220);  */
    
-- 호봉이나 직급이 변경되면 월급이 변경되는 프로시저를 만드세요
-- 트리거는 추가/변경된 행의 정보를 수정할 수 없음
...
begin 
    ...
    declare _add int default 0;  -- 호봉추가금 변수
    declare _salary int default 0; -- 월급 변수
    -- 기본급 가져오기
    set _base = (select sa_em_base_salary from salary where sa_level = _level);
    -- 호봉 기본급 가져오기
    set _add = (select sa_em_add_salary from salary where sa_level = _level);
    -- 연차에 맞게 호봉 추가금 계산하기
    set _add = _add * _year;
    -- 사원의 직급과 연차, 월급 수정
    update employee set em_sa_level = _level, em_year = _year, em_salary = _base + _add where em_num = _num;    
end //
delimiter ;	
